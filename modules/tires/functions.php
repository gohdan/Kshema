...
function tires_transliterate($string, $lang_from, $lang_to)
{
	...
	if ("ru" == $lang_from && "en" == $lang_to)
	{
			$new_string = strtr($string, 
				"àáâãäåæçèéêëìíîïğñòóôûÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÛ",
			    "abvgdejziyklmnoprstufyABVGDEJZIYKLMNOPRSTUFY"
			);
			$new_string = strtr($new_string, array(
			'¸'=>"yo",    'õ'=>"h",  'ö'=>"ts",  '÷'=>"ch", 'ø'=>"sh",  
			'ù'=>"sch",   'ú'=>'*',   'ü'=>'**', 'ı'=>"ye", 'ş'=>"yu", 'ÿ'=>"ya",
			'¨'=>"Yo",    'Õ'=>"H",  'Ö'=>"Ts",  '×'=>"Ch", 'Ø'=>"Sh",
			'Ù'=>"Sch",   'Ú'=>'***',   'Ü'=>'****', 'İ'=>"Ye", 'Ş'=>"Yu", 'ß'=>"Ya",
			'/'=>"--"
			));
	}
	else if ("en" == $lang_from && "ru" == $lang_to)
	{
			...
			'yo'=>"¸",    'h'=>"õ",  'ts'=>"ö",  'ch'=>"÷", 'sh'=>"ø",  
			'sch'=>"ù",   '*' => "ú", '**' => "ü", 'ye'=>"ı", 'yu'=>"ş", 'ya'=>"ÿ",
			'Yo'=>"¨",    'H'=>"Õ",  'Ts'=>"Ö",  'Ch'=>"×", 'Sh'=>"Ø",
			'Sch'=>"Ù",   '***' => "Ú", '****' => "Ü", 'Ye'=>"İ", 'Yu'=>"Ş", 'Ya'=>"ß",
			'--'=>"/"
			));
...
function tires_if_has_cyrillic($string)
{
	...
	//$letters = "àáâãäå¸æçèéêëìíîïğñòóôúûüışÿÀÁÂÃÄÅ¨ÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÚÛÜİŞß";
